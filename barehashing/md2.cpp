...
void MD2Update(MD2_CTX* context,unsigned char* input,unsigned int inputLen)
{
    ...
    if (inputLen >= partLen) 
    {
        MD2_memcpy((unsigned char*)&context->buffer[index], (unsigned char*)input, partLen);
    ...
    MD2_memcpy((unsigned char*)&context->buffer[index], (unsigned char*)&input[i],inputLen - i);
}
